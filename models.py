...
def ELMs(args, **kwargs):
    ...
    if args.prune_interval_start_layer_stats is not None:
        stats = torch.load(args.prune_interval_start_layer_stats, map_location='cpu')
...
def ELMs_GPPO(args, **kwargs):
    ...
    for name, param in model.named_parameters():
        if "policy_model" in name or "lora" in name:
            param.requires_grad = True
            param.data = param.data.float()
        else:
            param.requires_grad = False
